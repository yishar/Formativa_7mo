<?php

namespace frontend\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use common\models\Certificados;

/**
 * CertificadosSearch represents the model behind the search form about `common\models\Certificados`.
 */
class CertificadosSearch extends Certificados
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['id_certificado'], 'integer'],
            [['matricula', 'file_certificado'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Certificados::find();

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        $query->andFilterWhere([
            'id_certificado' => $this->id_certificado,
        ]);

        $query->andFilterWhere(['like', 'matricula', $this->matricula])
            ->andFilterWhere(['like', 'file_certificado', $this->file_certificado]);

        return $dataProvider;
    }
}
